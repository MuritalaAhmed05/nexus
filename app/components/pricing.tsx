...
const TicketCard: React.FC<TicketCardProps> = ({ title, description, price }) => {
  ...
  return (
    <div
      className={`rounded-[50px] border border-white overflow-hidden transition-colors duration-300 flex flex-col px-6 py-8 md:px-15 md:py-18
        ${isHovered ? "bg-blue-600" : "bg-black"} h-full`} // Adjust padding for mobile
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="p-4 md:p-8 flex-grow flex flex-col justify-between"> {/* Adjusted padding for mobile */}
  ...
  return (
    <div className="bg-black min-h-screen p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <h1 className="font-syne font-bold text-[30px] md:text-[50px] leading-none tracking-normal capitalize mb-8 md:mb-12">
          Ticket Options
        </h1>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-4">
          <TicketCard title="EARLY BIRD PASS" description="Limited time offer!" price="299" />
          <TicketCard title="STANDARD PASS" description="" price="399" />
          <TicketCard title="VIP EXPERIENCE" description="Includes exclusive speaker meetups & front-row seating" price="699" />
        </div>
      </div>
    </div>
  );
